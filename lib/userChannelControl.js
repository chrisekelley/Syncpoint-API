    ...
    function setupChannelDb(doc) {
      validateChannelRequest(doc, function() {
        coux([config.host,"_uuids"], e(function(err, data) {
         ...
         // create the todos4all couchapp
            console.log("create todos4all couchapp in channel",doc.cloud_database,"for user",doc.owner_id);
            coux.put([config.host, doc.cloud_database], function() {
                couchapp.createApp(require('./design/todos4all'), 
                  [config.host, doc.cloud_database].join('/'), function(app) {
                    app.push()
                })
              })
        }));
